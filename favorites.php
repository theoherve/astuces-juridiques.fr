...
if($_SERVER['REQUEST_URI']=="/favorites.php"){
  $_SERVER['REQUEST_URI'].="?";
}


$reqFavAll = $db->prepare('SELECT id_favorite FROM favorites WHERE id_user=:id_user');
$reqFavAll->bindParam(':id_user', $_SESSION['id'], PDO::PARAM_INT);
$reqFavAll->execute();
$allAstuces = $reqFavAll->rowCount();

$reqFav = $db->prepare('SELECT * FROM favorites WHERE id_user=:id_user ORDER BY id_favorite DESC LIMIT :num_page, 10');
$reqFav->bindParam(':id_user', $_SESSION['id'], PDO::PARAM_INT);
$reqFav->bindParam(':num_page', $num_page, PDO::PARAM_INT);
$reqFav->execute();
$favorites=$reqFav->fetchAll(PDO::FETCH_ASSOC);


$reqUser = $db->prepare('SELECT * FROM users WHERE id_user=:id_user');
$reqUser->bindParam(':id_user', $_SESSION['id'], PDO::PARAM_INT);
$reqUser->execute();
  ...
  <body>
    ...
    <main>
      ...
      <?php foreach ($favorites as $favorite):
        $reqAstuces = $db->prepare('SELECT * FROM articles WHERE status="2" AND id_article=:id_article ORDER BY release_date DESC');
        $reqAstuces->bindParam(':id_article', $favorite['id_article'], PDO::PARAM_INT);
        $reqAstuces->execute();
        $astuce = $reqAstuces->fetch(PDO::FETCH_ASSOC);
        ...
        <div class="astuceContainer">
          <div class="astuceHeader" margin="yes">
            ...
            <?php if($connected == 1){ ?>
              ...
              <?php }else{ ?>
                <a onclick="unlikeReload(<?php echo $astuce['id_article'] . ',' . $_SESSION['id'];?>)"><div id="imgFavorisDone<?php echo $astuce['id_article']?>"><img src="pictures/favoris_done.png" alt="Favoris" style="cursor: pointer;"></div></a>
              <?php } ?>
            <?php }else { ?>
          ...
          <div class="astuceSocial" margin="yes">
            ...
            ?>
            <!-- <a id="buttonComment" onclick="display_comments(<?php // echo $astuce['id_article'] . "," . $_SESSION['id']?>)" style="cursor: pointer;">Commentaires</a> -->
            <a id="buttonComment" onclick="openClose(<?php echo $astuce['id_article'] ?>)" style="cursor: pointer;">Commentaires <?php echo "(" . $allComments . ")"; ?></a>
